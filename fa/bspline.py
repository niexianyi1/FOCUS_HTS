...
#### 三阶导数的前2个点和后三个点的数值是否按照解析公式计算，还是与scipy一样重复

## 在这里需要一个重合点，输入时自带
def get_c_init(init_coil, nc, ns):       
    coil = np.load("{}".format(init_coil))[:, :ns+1, :]
    c, bc  = tcku(coil, nc, ns+1, 3)
...
def der3_splev(bc, wrk2):    # 三阶导数    
    ...
    m = len(u)  
    wrk3 = np.zeros((3, m-3))
    wrk3 = wrk3.at[:, :].set((wrk2[:, 1:]-wrk2[:, :-1])/(1/(m-3)))
    j1 = int(np.ceil(m/2))
    der3 = np.zeros((m,3))
    der3 = der3.at[0, :].set(wrk3[:, 0].T)
    for i in range(1, j1):
        der3 = der3.at[i, :].set(wrk3[:, i-1].T)
    for i in range(j1, m-1):
        der3 = der3.at[i, :].set(wrk3[:, i-2].T)
    der3 = der3.at[m-1, :].set(wrk3[:, m-4].T)    

    return der3

# def close(bc, c):
#     xyz = vmap(lambda c :splev(bc, c), in_axes=0, out_axes=0)(c)
#     coil = np.zeros_like(xyz)
#     coil = coil.at[:, :-1, :].set(xyz[:, 1:, :])
#     coil = coil.at[:, -1, :].set(xyz[:, 1, :])
#     # xyz = xyz.at[:, -1, :].set(xyz[:, 0, :])
#     c_new, _ = tcku(coil, coil.shape[0], coil.shape[1], 3)
#     c_new = c_new .at[:, :, -3:].set(c_new [:, :, :3])
#     return c_new
