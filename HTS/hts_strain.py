...
def cn(args, coil_output_func, params):
    _, dl, _, der1, der2, _, v1, _, _ = coil_output_func(params)
    curva = np.cross(der1, der2) / (np.linalg.norm(der1, axis = -1)**3)[:,:,np.newaxis]
    strain = HTS_strain(args, curva, v1, dl)
    strain_max = np.max(strain)-args['target_HTS_strain']
    return strain_max 


def HTS_strain(args, curva, v, dl):
    width = args['HTS_signle_width']
    dl = np.mean(dl[:args['number_independent_coils']], axis=(1,2))
    bend = HTS_strain_bend(width, curva, v)
    tor = HTS_strain_tor(width, dl, v)
    
    return bend + tor

def HTS_strain_bend(width, curva, v):
    """弯曲应变,
    Args:
        w, 带材宽度
        v,有限截面坐标轴
...
def HTS_strain_tor(width, dl, v):
    ...
    # cosv = cosv.at[:, :-1].set(np.sum(v[:, :-1, :] * v[:, 1:, :], axis=-1) + 1e-8) # 此处分母都为1，省略
    # cosv = cosv.at[:, -1].set(np.sum(v[:, -1, :] * v[:, 0, :], axis=-1) + 1e-8)
