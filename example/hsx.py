...
# 优化
    ...
    'step_size_coil':           4e-5,       #       float,  参数coil_arg, 迭代步长, 为0则不优化                      
...
# 线圈
    ...
    'circle_coil_radius':       0.45,        #       float,  生成初始的线圈半径   
    'coil_file_type':           'makegrid',      #       str,    初始线圈文件类型, 'npy' or 'makegrid', 后续可以再加
    'init_coil_file':                       #       str,    初始线圈文件名
            '/home/nxy/codes/coil_spline_HTS/initfiles/hsx/coils.hsx',     
    'read_coil_segments':        385,        #       int,    文件中的线圈段数 

    # 线圈参数
    'number_coils':             48,         # (nc)  int,    线圈总数                    
    'number_field_periods':     4,          # (nfp) int,    线圈周期数                     
    'stellarator_symmetry':     1,          # (ss)  int,    仿星器对称，1:对称，0:非对称                     
    'number_independent_coils': 6,          # (nic) int,    独立线圈数(半周期线圈数), (nc=nfp*(ss+1)*nic)                     
    ...
    'length_normal':            [0.04 for i in range(6)],       # (ln)  list,  有限截面下每个线圈的法向间隔的长度                   
    'length_binormal':          [0.04 for i in range(6)],       # (lb)  list,  有限截面下每个线圈的副法向间隔的长度                    
...
# 磁面
    ...
    'surface_vmec_file':                    #       str,    磁面数据的vmec文件
            '/home/nxy/codes/coil_spline_HTS/initfiles/hsx/plasma.boundary',
...
# loss function 目标和权重
    ...
    'weight_length':            0.1,          # (wl)  float,  单根线圈平均长度 
    'weight_curvature':         4e-4,          # (wc)  float,  曲率 
    'weight_curvature_max':     4e-3,          # (wcm) float,  最大曲率
    'weight_torsion':           2e-4,          # (wt)  float,  扭转 
    'weight_torsion_max':       4e-3,          # (wtm) float,  最大扭转
    'weight_distance_coil_coil':    0.1,      # (wdcc)float,  线圈间距 
    'weight_distance_coil_surface': 0.05,      # (wdcs)float,  线圈与磁面距离 
    'weight_strain':            0.1,          #       float,  应变量
    'weight_force':             0,          # (wf)  float,  线圈受自场力  
    'weight_jcrit':             0,          #       float,  线圈自场与线圈表面夹角   


    #  target 
    'target_length':            2.2,          #       float,  目标长度, 
    'target_curvature_max':     5,          #       float,  目标最大曲率
    'target_torsion_max':       10,          #       float,  目标最大扭转
    'target_distance_coil_coil':    0.08,      #       float,  目标最大线圈间距
    'target_distance_coil_surface': 0.14,      #       float,  目标最大线圈与磁面距离
    'target_strain':            0.02,          #       float,  目标最大应变
    'target_force':             1,
...
# 程序输出
    ...
    'out_hdf5':                             #       str,    hdf5, 输出参数
        '/home/nxy/codes/coil_spline_HTS/results/hsx/hsx.h5',        
