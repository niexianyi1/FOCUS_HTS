...
# 线圈
    ...
    'circle_coil_radius':       0.75,        #       float,  'circle' 的初始半径   
    
    # 线圈文件
    'init_coil_file':                       #       str,    初始线圈文件名
            'initfiles/QA/coils.pqa_well',       
...
# 磁面
    ...
    'surface_vmec_file':                    #       str,    磁面数据的vmec文件
            'initfiles/QA/plasma_well.boundary',
...
# loss function 目标和权重
    ...
    'weight_distance_coil_coil':    2e-3,      # (wdcc)float,  线圈间距 
    'weight_distance_coil_surface': 1e-4,      # (wdcs)float,  线圈与磁面距离 
    'weight_HTS_strain':            0,          #       float,  应变量,一般设为0,若想优化应变可通过不等式约束
    ...
    'target_length_single':                     #  list/float,   每个线圈目标长度,若第一项为0则不约束
                            [0, 5, 5, 4.8],
    'target_curvature_max':     4,          #       float,  目标最大曲率
    'target_torsion_max':       20,          #       float,  目标最大扭转
    'target_distance_coil_coil':    0.1,      #       float,  目标最大线圈间距
    'target_distance_coil_surface': 0.3,      #       float,  目标最大线圈与磁面距离
    'target_HTS_strain':            0.008,          #       float,  目标最大应变
...
# 结果保存
    ...
    'out_hdf5':                             #       str,    hdf5, 输出参数
        'results/QA/circle_start/current/f10n6_well_2.h5',   
