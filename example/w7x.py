...
# 优化
    ...
    'optimizer':           'momentum', # (opt) str,    线圈参数迭代方法,  (sgd, gd, momentum, or adam)  
...
# LTS/HTS材料(为方便下列参数都以HTS命名)
    ...
    'HTS_single_width':         4e-3,       # (sw)  float,  HTS材料单根宽度
    'HTS_single_thickness':     5e-5,       # (st)  float,  HTS材料单层结构厚度
...
# loss function 目标和权重
    ...
    'weight_length':            0.05,          # (wl)  float,  单根线圈平均长度 
    'weight_curvature':         0.0002,          # (wc)  float,  曲率 
    'weight_curvature_max':     0.001,          # (wcm) float,  最大曲率
    'weight_torsion':           0.0002,          # (wt)  float,  扭转 
    'weight_torsion_max':       0.001,          # (wtm) float,  最大扭转
    'weight_distance_coil_coil':    0.001,      # (wdcc)float,  线圈间距 
    'weight_distance_coil_surface': 0.01,      # (wdcs)float,  线圈与磁面距离 
    'weight_HTS_strain':            0,          #       float,  应变量
    'weight_HTS_force_max':      4e-9,          # (wf)  float,  线圈受自场力  
    'weight_HTS_force_mean':     9e-9,          # (wf)  float,  线圈受自场力   
    'weight_HTS_Icrit':             0,          #       float,  线圈自场与线圈表面夹角   

    # 目标
    'target_length_mean':            8.46,          #       float,  目标长度, 若为0则约束其较小
    ...
    'target_distance_coil_coil':    0.23,      #       float,  目标最大线圈间距
    'target_distance_coil_surface': 0.4,      #       float,  目标最大线圈与磁面距离
    'target_HTS_strain':            0.004,          #       float,  目标最大应变
    'target_HTS_force_max':             6.9e6,            #       float, 目标最大受力
