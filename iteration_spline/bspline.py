...
def tjev(bc):
    """获取节点序列,在后续计算中无需重复"""
    t, u, _ = bc
    t0 = numpy.zeros_like(t[0])
    u0 = numpy.zeros_like(u[0])
    tj = numpy.zeros_like(u)  
    for a in range(len(t)):         # len(t) = nic
        j = 0
        for i in range(len(t[a])):
            t0[i] = t[a, i]
        for i in range(len(u[a])):  # len(u[a]) = ns
            u0[i] = u[a, i]
            while u0[i]>=t0[j+1] :
                j = j+1
            tj[a, i] = j
...
def prep(rc, nic, ns, ncp, k):  
    ...
    t = np.zeros((nic, ncp+4))
    c = np.zeros((nic, 3, ncp))   
    u = np.zeros((nic, ns))
    for i in range(nic):
        tck, u0 = si.splprep(x = x[i], k=3, s=0, per=1)
        t = t.at[i,:].set(tck[0])
        c = c.at[i,:,:].set(tck[1])
        u = u.at[i,:].set(u0[:-1])
    # u = np.linspace(0, (ns-1)/ns ,ns)
...
def der1_splev(t, u, c, tj, ns):       # 一阶导数  
    ...
    c = np.array(c)
    wrk1 = wrk1.at[:, :].set(3*(c[:, 1:]-c[:, :-1])/(3/(ncp-3)))
    t = np.delete(t, 0)
    der1 = np.zeros((ns,3))
...
def der2_splev(t, u, wrk1, tj, ns):       # 一阶导数   
    ...
    wrk2 = wrk2.at[:, :].set(2*(wrk1[:, 1:]-wrk1[:, :-1])/(2/(ncp-3)))
    t = np.delete(t, 0)
    t = np.delete(t, 0)
    der2 = np.zeros((ns,3))
